...
public abstract class OptimizedAsyncTask<Params, Progress, Result> {
    private static final String LOG_TAG = "OptimizedAsyncTask";

    private static final int CORE_POOL_SIZE = 10;//modify

    private static final ThreadFactory sThreadFactory = new ThreadFactory() {
        private final AtomicInteger mCount = new AtomicInteger(1);

        public Thread newThread(Runnable r) {
            return new Thread(r, "OptimizedAsyncTask #" + mCount.getAndIncrement());
        }
    };

    private static final ExecutorService sExecutor = Executors.newFixedThreadPool(CORE_POOL_SIZE, sThreadFactory);
        ...
        mFuture = new FutureTask<Result>(mWorker) {
            ...
            protected void done() {
                ...
                try {
                    ...
                    message = sHandler.obtainMessage(MESSAGE_POST_CANCEL, new PreReadTaskResult<Result>(OptimizedAsyncTask.this, (Result[]) null));
                ...
                message = sHandler.obtainMessage(MESSAGE_POST_RESULT, new PreReadTaskResult<Result>(OptimizedAsyncTask.this, result));
                message.sendToTarget();
            }
        };
    }
     ...
     * returned by {@link #doInBackground} or null if the task was cancelled or an exception
     * occured.
     * 
     ...
     */
    public final OptimizedAsyncTask<Params, Progress, Result> execute(Params... params) {
    ...
    protected final void publishProgress(Progress... values) {
        sHandler.obtainMessage(MESSAGE_POST_PROGRESS, new PreReadTaskResult<Progress>(this, values)).sendToTarget();
    }
    ...
    private static class PreReadTaskResult<Data> {
        final OptimizedAsyncTask mTask;
        final Data[] mData;

        PreReadTaskResult(OptimizedAsyncTask task, Data... data) {
            mTask = task;
            mData = data;
        }
    }
