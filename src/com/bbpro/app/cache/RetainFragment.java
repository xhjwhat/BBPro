...
public class RetainFragment extends Fragment {
    private static final String TAG = "RetainFragment_";
     ...
     */
    public static RetainFragment findOrCreateRetainFragment(FragmentManager fm, String uniqueName) {
        // Check to see if we have retained the worker fragment.
        RetainFragment mRetainFragment = (RetainFragment) fm.findFragmentByTag(TAG + uniqueName);

        // If not retained (or first time running), we need to create and add it.
        if (mRetainFragment == null) {
            mRetainFragment = new RetainFragment();
            fm.beginTransaction().add(mRetainFragment, TAG + uniqueName).commit();
        }
