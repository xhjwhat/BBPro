...
public class DiskLruCache {
	...
	private static final String CACHE_FILENAME_SUFFIX = ".cache";
	...
	private static final FilenameFilter cacheFileFilter = new FilenameFilter() {
		...
		public boolean accept(File dir, String filename) {
			return filename.endsWith(CACHE_FILENAME_SUFFIX);
		}
	};
	...
	public static DiskLruCache openCache(File cacheDir, long maxByteSize) {
		return openCache(null, cacheDir, maxByteSize);
	}
	...
	public File get(String key) {
		synchronized (mLinkedHashMap) {
			...
			if (path != null) {
				...
				final String existPath = createFilePath(mCacheDir, key);
	...
	public static File getDiskCacheDir(Context context, String uniqueName) {
		...
		final String cachePath = Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED) || !Utils.isExternalStorageRemovable() ? Utils.getExternalCacheDir(context).getPath()
	...
	public static String createFilePath(File cacheDir, String key) {
		try {
			...
			// but it will do for
			// this example
			return cacheDir.getAbsolutePath() + File.separator + URLEncoder.encode(key.replace("*", "") + CACHE_FILENAME_SUFFIX, "UTF-8");
